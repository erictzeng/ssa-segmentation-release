...
class ASPP(nn.Module):
    ...
    def forward(self, x):
        ...
        x5 = self.aspp5_bn(x5)
        x5 = self.relu(x5)
        x5 = F.interpolate(x5, size=(x.shape[2], x.shape[3]), mode='bilinear',
                           align_corners=True)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes, num_groups=None, weight_std=False, beta=False, freeze=False, out_dim=256):
        self.inplanes = 64
        ...
        self.layer4 = self._make_layer(block, 512, layers[3], stride=1,
                                       dilation=2)
        self.aspp = ASPP(512 * block.expansion, out_dim, num_classes, conv=self.conv, norm=self.norm)
        self.out_dim = out_dim
        self.not_training = [self.layer1, self.layer2, self.layer3, self.layer4]
...
def resnet101(pretrained=False, num_groups=None, weight_std=False, **kwargs):
    ...
    if pretrained:
        ...
        print(model_dict['aspp.aspp1.weight'].size())
        if num_groups and weight_std:
            ...
            overlap_dict = {k: v for k, v in pretrained_dict.items() if k in model_dict and v.size() == model_dict[k].size()}
